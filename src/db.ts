...
CREATE INDEX IF NOT EXISTS idx_tasks_tenant ON tasks(board_id, tenant);
      ...
      const migrate = dbInstance.transaction(() => {
        dbInstance!.exec(`
          ...
          INSERT INTO tasks_new
            (id, board_id, title, body, assignee, status, priority, tenant, workspace_kind, branch, result, summary, block_reason, schedule_reason, review_reason, scheduled_at, skills, created_at, updated_at, started_at, archived_at, current_run_id, claim_lock, claim_expires, last_heartbeat_at, max_runtime_seconds, idempotency_key)
          SELECT
            ...
            tenant, workspace_kind, branch, result, summary, block_reason, schedule_reason, review_reason, scheduled_at, skills, created_at, updated_at, started_at, archived_at, current_run_id, claim_lock, claim_expires, last_heartbeat_at, max_runtime_seconds, idempotency_key
          FROM tasks;
          DROP TABLE tasks;
