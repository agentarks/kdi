...
| `ff_worktree_handoff` | `FF_WORKTREE_HANDOFF` | CLI / dispatcher | InDev | `false` | KDI-055 | Preserve successful task worktrees/branches when they contain local changes or commits; emit handoff event with branch and worktree path.
...
- **Status transitions:**
  - `Planned` → `InDev` when Hermes-parity display-name rename and `rename-slug` command are implemented (done).
- **Activation criteria:**
...
### `ff_worktree_handoff` — InDev

- **Owner:** kdi core team
- **BRD:** [BRD-KDI-055](brd-kdi-055-worktree-handoff.md)
- **Status transitions:**
