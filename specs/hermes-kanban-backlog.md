...
- [x] **KDI-052: Pass task title/body to harness**  
  Implemented behind `FF_HARNESS_CONTEXT`. `ALLOWED_TEMPLATES` and `substituteCommand` support `{{title}}`/`{{body}}`; dispatcher exports `KDI_TASK_TITLE`, `KDI_TASK_BODY`, `KDI_TASK_ID`, and `KDI_BOARD` to the harness when the flag is enabled.
