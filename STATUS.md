...
- [x] `bun run lint` (CLI tsc --noEmit clean), `bun run build` (CLI 122 modules), `bun run check:web` (svelte-check 0/0), `bun run build:web` (adapter-node, `bun:sqlite` emitted external), and `bun test` (970 pass / 0 fail) all pass with isolated `KDI_DB`. `src/` byte-identical (no changes to `src/models/*`, `src/commands/*`, `src/db.ts`, `src/flags.ts`).
