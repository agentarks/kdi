...
- [x] **`tests/init.test.ts` fails when `KDI_DB` is set** — Fixed by asserting `defaultDbPath()` honors `KDI_DB`/`KDI_DB_PATH` override paths (including `.sqlite`) and by restoring KDI DB env vars in tests that override them.
